...
def compile_director_ir(director_ir: Dict[str, Any]) -> Dict[str, Any]:
    segments=director_ir.get("segments",[]); duration=max((float(s.get("end",0)) for s in segments),default=.001)
    layers=[]; subtitles=[]; audio=[]; cameras=[]
    for s in segments:
        start=float(s.get("start",0)); end=float(s.get("end",start)); caption=s.get("caption_intent") or {}; motion=s.get("motion_intent") or {}; camera=s.get("camera_intent") or {}; a=s.get("audio_intent") or {}; text=s.get("transcript") or ""
    ...
    return {"version":"1.0","canvas":{"width":1920,"height":1080,"fps":30,"background":"#000000"},"scenes":[{"id":"director-scene-1","duration":max(duration,.001),"camera":{"movement":cameras[0] if cameras else "none"},"layers":layers,"subtitle_cues":subtitles,"audio_cues":audio}]}
